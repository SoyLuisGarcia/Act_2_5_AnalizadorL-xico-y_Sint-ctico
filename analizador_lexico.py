...
def t_RESERVADA(t):
    r'\b(for|FOR|int|INT|void|VOID|return|RETURN|main|MAIN|static|STATICs)\b'
    t.type = 'RESERVADA'
    #r'for'
    return t
...
def categorizar_token(tipo):
    if tipo == 'RESERVADA':
        ...
        counts['NUMERO'] += 1
    elif tipo in ('PUNTO', 'NUMERAL', 'COMDOB'):
    #elif tipo == 'PUNTO':
        counts['SIMBOLO'] += 1
...
def identificar_token(token):
    # Aquí agregas la lógica para identificar el tipo de cada token
    if token in ['programa', 'int', 'read', 'printf', 'end']:
        return 'RESERVADO'
    elif token in ['(', ')', '{', '}', ';']:
        return 'DELIMITADOR'
    elif token.isdigit():
...
def analizar_lexico(codigo_fuente):
    ...
    for token in codigo_fuente.split():
        tipo_token = identificar_token(token)  # Función que identifica el tipo de token
        tokens.append({'token': token, 'tipo': tipo_token})
        
        # Aumenta el conteo basado en el tipo de token
        if tipo_token in counts:
            counts[tipo_token] += 1
